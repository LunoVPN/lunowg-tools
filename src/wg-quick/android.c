...
#define AWG_PACKAGE_NAME "org.lunovpn.awg"
#endif
#ifndef AWG_CONFIG_SEARCH_PATHS
#define AWG_CONFIG_SEARCH_PATHS "/data/misc/lunovpn /data/data/" AWG_PACKAGE_NAME "/files"
#endif
...
{
	if (is_awg_on)
		cmd("lunovpn-go %s", iface);
	else
		cmd("ip link add %s type lunovpn", iface);
}
...
static void del_if(const char *iface)
{
	...
	_cleanup_free_ char *iptables_regex = concat("^-A (.* --comment \"lunovpn rule ", iface, "\"[^\n]*)\n*$", NULL);
...
static bool should_block_ipv6(const char *iface)
{
	...
	for (char *endpoint = cmd_ret(&c, "lwg show %s endpoints", iface); endpoint; endpoint = cmd_ret(&c, NULL)) {
...
static uint16_t determine_listen_port(const char *iface)
{
	...
	value = cmd_ret(&c, "lwg show %s listen-port", iface);
	if (!value)
		goto set_back_down;
...
static void up_if(unsigned int *netid, const char *iface, uint16_t listen_port)
{
	...
	cmd("lwg set %s fwmark 0x20000", iface);
	cmd("iptables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"lunovpn rule %s\"", iface);
	cmd("ip6tables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"lunovpn rule %s\"", iface);
	if (listen_port) {
		cmd("iptables -I INPUT 1 -p udp --dport %u -j ACCEPT -m comment --comment \"lunovpn rule %s\"", listen_port, iface);
		cmd("ip6tables -I INPUT 1 -p udp --dport %u -j %s -m comment --comment \"lunovpn rule %s\"", listen_port, should_block_ipv6(iface) ? "DROP" : "ACCEPT", iface);
	}
	cmd("ip link set up dev %s", iface);
...
static void set_mtu(const char *iface, unsigned int mtu)
{
	...
	for (char *endpoint = cmd_ret(&c_endpoints, "lwg show %s endpoints", iface); endpoint; endpoint = cmd_ret(&c_endpoints, NULL)) {
...
static void set_routes(const char *iface, unsigned int netid)
{
	...
	for (char *allowedips = cmd_ret(&c, "lwg show %s allowed-ips", iface); allowedips; allowedips = cmd_ret(&c, NULL)) {
...
{
	FILE *config_writer;
	_cleanup_free_ char *cmd = concat("lwg setconf ", iface, " /proc/self/fd/0", NULL);
	int ret;
...
static void broadcast_change(void)
{
	const char *pkg = getenv("CALLING_PACKAGE");
		...
		cmd("am broadcast -a org.lunovpn.awg.action.REFRESH_TUNNEL_STATES " AWG_PACKAGE_NAME);
}
...
static void cmd_usage(const char *program)
{
	...
	printf( "\n  It is to be readable by lwg(8)'s `setconf' sub-command, with the exception\n"
		...
		"  See lwg-quick(8) for more info and examples.\n");
}
...
static void cmd_down(const char *iface)
{
	...
	char *ifaces = cmd_ret(&c, "lwg show interfaces");
	if (ifaces) {
