...
parse_options() {
	...
	CONFIG_FILE="$1"
	[[ $CONFIG_FILE =~ ^[a-zA-Z0-9_=+.-]{1,15}$ ]] && CONFIG_FILE="/etc/lunovpn/wg/$CONFIG_FILE.conf"
...
add_if() {
	local ret
	if ! cmd ip link add "$INTERFACE" type lunovpn; then
		ret=$?
		[[ -e /sys/module/lunovpn ]] || ! command -v "${WG_QUICK_USERSPACE_IMPLEMENTATION:-lunovpn-go}" >/dev/null && exit $ret
		echo "[!] Missing WireGuard (LunoVPN) kernel module. Falling back to slow userspace implementation." >&2
		cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-lunovpn-go}" "$INTERFACE"
	fi
}
...
del_if() {
	...
	if [[ -z $TABLE || $TABLE == auto ]] && get_fwmark table && [[ $(lwg show "$INTERFACE" allowed-ips) =~ /0(\ |$'\n'|$) ]]; then
...
set_mtu_up() {
	...
	done < <(lwg show "$INTERFACE" endpoints)
	if [[ $mtu -eq 0 ]]; then
...
get_fwmark() {
	local fwmark
	fwmark="$(lwg show "$INTERFACE" fwmark)" || return 1
	...
	if type -p nft >/dev/null; then
		...
		for iptables in iptables ip6tables; do
			...
			while read -r line; do
				[[ $line == "*"* || $line == COMMIT || $line == "-A "*"-m comment --comment \"lwg-quick(8) rule for $INTERFACE\""* ]] || continue
				[[ $line == "-A"* ]] && found=1
...
add_default() {
	...
	if ! get_fwmark table; then
		...
		cmd lwg set "$INTERFACE" fwmark $table
	fi
	...
	local marker="-m comment --comment \"lwg-quick(8) rule for $INTERFACE\"" restore=$'*raw\n' nftable="wg-quick-$INTERFACE" nftcmd
...
set_config() {
	cmd lwg setconf "$INTERFACE" <(echo "$WG_CONFIG")
}
...
save_config() {
	...
	old_umask="$(umask)"
	umask 077
	current_config="$(cmd lwg showconf "$INTERFACE")"
...
cmd_usage() {
	...
	Usage: $PROGRAM [ up | down | save | strip ] [ CONFIG_FILE | INTERFACE ]
	  ...
	  configuration found at /etc/lunovpn/wg/INTERFACE.conf. It is to be readable
	  by lwg(8)'s \`setconf' sub-command, with the exception of the following additions
...
cmd_up() {
	...
	set_mtu_up
	set_dns
	for i in $(while read -r _ i; do for i in $i; do [[ $i =~ ^[0-9a-z:.]+/[0-9]+$ ]] && echo "$i"; done; done < <(lwg show "$INTERFACE" allowed-ips) | sort -nr -k 2 -t /); do
		add_route "$i"
	done
...
cmd_down() {
	[[ " $(lwg show interfaces) " == *" $INTERFACE "* ]] || die "\`$INTERFACE' is not a WireGuard interface"
